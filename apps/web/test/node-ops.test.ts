...
import { addChild, moveNode, removeAt, updateAt } from '../src/lib/builder/node-ops';
...
describe('node-ops', () => {
  test('addChild appends to group at path', () => {
    ...
    expect((next as { children: QueryNode[] }).children.map((c) => (c as { value: string }).value)).toEqual([
      'a',
      'b',
    ]);
  });
  ...
  test('updateAt replaces node deeply', () => {
    const t = G([G([T('inner')])]);
    const next = updateAt(t, [0, 0], (n) => ({
      ...(n as { type: 'term'; value: string }),
      value: 'updated',
    }));
